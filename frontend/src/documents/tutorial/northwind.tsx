...
export const slides = [
  <div className="row content-row">
    ...
    <div className="col-sm-9">
      ...
      <p>
        본 튜토리얼은 사용자들이 AgensGraph에 샘플 데이터를 적재하고 적재된 데이터를 조회하거나 원하는 정보를 추출하는
        과정을 따라할 수 있도록 안내하기 위해 제작되었습니다.
      </p>
      <p>
        본 튜토리얼을 따라 데이터를 적재하고 활용하시려면 반드시 AgensGraph를 먼저 설치하셔야 하며 시각화 도구인
        AGViewer 함께 설치해 사용하신다면 더 쉽고 빠르게 샘플 데이터를 이해하고 분석하실 수 있습니다.
      </p>
      ...
      <p>
        이 튜토리얼에서는 데이터셋 파일(CSV)을 AgensGraph로 Import하는 방법을 설명할 것이며, 쿼리로 데이터를 생성하는
        방법이 궁금하시다면 Bitnine 홈페이지를 방문해
        <a target="_blank" rel="noopener noreferrer" href="http://bitnine.net/learn/">
          {' '}
          Basic Cypher & Advanced Cypher
        </a>
        백서를 참조하시기 바랍니다. 본 튜토리얼은 AgensGraph의 데이터 생성, 읽기, 업데이트, 삭제(CRUD) 결과를 시각화
        도구인 AGViewer를 통해 안내합니다. 또한 본 튜토리얼에서 안내하는 제품 일부 기능은 오직 AgensGraph Enterprise
        Edition에서만 제공됩니다. Enterprise 제품에 관한 자세한 사항은 Bitnine 홈페이지를 참조하시기 바랍니다.
      </p>
    </div>
  </div>,
  <div className="row content-row" key="s2">
    <div className="col-sm-3">
      ...
      <p>본 튜토리얼에서는 RDBMS에서 널리 사용되는 샘플 데이터셋인 Northwind 데이터셋을 사용합니다.</p>
      <hr />
      <p>
        <small>:help</small>{' '}
    ...
    <div className="col-sm-9">
      <h4>Load records</h4>
      <p>
        Northwind 데이터셋은 흔히 관계형 데이터 예제로 사용되지만 그래프 데이터 예제로도 사용할 수 있습니다. Northwind
        데이터셋에는 전세계에서 특수 식품을 수입, 수출하는 Northwind Traders라는 가상의 회사가 존재하며 이 회사의 판매
        데이터가 들어 있습니다. 본 튜토리얼에서 제공하는 Northwind 데이터셋은 총 11개 테이블, Vertex Label 6개, Edge
        Label 6개로 구성되어 있으며 고객 91명, 주문이력 830회, 판매된 상품 77가지를 포함하고 있습니다.
      </p>
      <p>
  ...
  <div className="row content-row" key="s3">
    <div className="col-sm-3">
      ...
      <p>
        그래프 모델(Graph Model)은 데이터를 Row와 Column으로 구성된 테이블(Table)이 아니라, 점(Node 또는 Vertex)과 점이
        선(Link 또는 Edge)로 연결된 형태인 그래프로 구현한 데이터 모델입니다.
      </p>
      <hr />
      <p>
        <small>:help</small>{' '}
    ...
    <div className="col-sm-9">
      ...
      <p>테이블을 사용하는 관계형 모델에서 그래프 모델로 전환 시 다음 내용을 참고하십시오.</p>
      <ol className="big">
      ...
      <p>
        ...
        <img src="resources/images/northwind/Graphmodel.png" className="img-responsive" alt="Graphmodel" />
      </p>
      <p>
        그래프 모델과 관계형 모델의 가장 큰 차이 중 하나는 그래프 모델이 관계형 모델과 다르게 현실 모델(비즈니스 모델)을
        있는 그대로 반영한다는 점입니다.{' '}
      </p>
      <p>관계형 모델은 복잡한 현실 세계의 비즈니스 모델을 가공해 표 형태의 테이블 형식으로 변환합니다. </p>
      <p>
        하지만 현실 세계의 비즈니스 모델은 각 객체(Vertex)간에 관계(Edge)가 맺어진 형태이고, 유사한 객체끼리의 묶음이
        그룹(Label)으로 표현되어 있습니다. 그리고 이는 그래프 모델의 데이터 구현 방식과 매우 유사합니다.{' '}
      </p>
    </div>
  </div>,
  <div className="row content-row" key="s6">
    ...
    <div className="col-sm-9">
      ...
      <p>FDW와 COPY명령어 모두, CSV파일을 Table화 하는데에 사용 되며 각 특징은 아래와 같습니다.</p>
      ...
      <p>
        이 튜토리얼에서는 AGViewer와 AgensGraph가 다른 서버에 설치되어 있다고 가정합니다. 따라서 사용자가 AgensGraph에서
        Import할 CSV파일을 업로드 할 수도, 절대 경로를 제공 할 수도 없다는 상황을 가정하기에, COPY명령어를 통하여
        Import하는 튜토리얼을 진행합니다. FDW를 사용한 Import방식이 궁금하시면 Bitnine 홈페지이를 방문해
        <a target="_blank" rel="noopener noreferrer" href="https://bitnine.net/tutorial/tutorial_kor.html">
          튜토리얼
        </a>{' '}
        를 참조하시기 바랍니다.
      </p>
  ...
  <div className="row content-row" key="s7">
    <div className="col-sm-3">
      ...
      <p>COPY Command를 사용하여 CSV데이터를 Table에 적재합니다.</p>
      <hr />
      <p>
        <small>:help</small>{' '}
        <a href="/#" className="badge badge-light">
    ...
    <div className="col-sm-9">
      <h4>Create Tables</h4>
      <figure>
        <aside className="warn">CSV 파일 형태의 데이터셋을 적재할 각각의 Table을 생성합니다.</aside>
        <CodeMirror
          options={{
            mode: 'cypher',
            readOnly: true,
...
CREATE TABLE territories (TerritoryID varchar(20), TerritoryDescription char(50), RegionID int);`}
      ...
      <figure>
        <aside className="warn">생성 된 테이블에 CSV 데이터셋을 적재합니다.</aside>
        <CodeMirror
          options={{
            mode: 'cypher',
            readOnly: true,
...
FROM PROGRAM 'curl https://raw.githubusercontent.com/KarlJeong/northwind_dataset/master/territories.csv' csv header delimiter ',';`}
  ...
  <div className="row content-row" key="s9">
    <div className="col-sm-3">
      <h3>Import CSV to AgensGraph via COPY</h3>
      <p>적재된 Table의 데이터를 기반으로 Vertex 생성하고 효율적인 탐색을 위하여 Index를 생성합니다.</p>
      <hr />
      <p>
        <small>:help</small>{' '}
        <a href="/#" className="badge badge-light">
...
LOAD FROM territories AS source CREATE (n:territory=to_jsonb(source));`}
        ...
        <aside className="warn">
          각 Table의 데이터를 로드하여 Vertex를 생성하였습니다. 이 때, Table 각 Column이 Vertex의 Property로 입력
          됩니다.
        </aside>
      </figure>
...
CREATE PROPERTY INDEX ON territory(territoryid);`}
        />
        <aside className="warn">생성한 Vertex별로 특정 속성에 Index를 생성합니다.</aside>
      </figure>
    </div>
  </div>,
...
CREATE (n)-[r:SOLD]->(m);`}
        />
        <aside className="warn">
          위 과정을 모두 정상적으로 수행하셨다면 아래와 같은 그래프 모델이 생성됩니다. (AGViewer에서 쿼리 수행 시 그래프
          레이아웃 및 쿼리 조건에 따라 보여지는 모습이 달라질 수 있습니다)
        </aside>
      </figure>
    </div>
  </div>,
  <div className="row content-row" key="s11">
    <div className="col-sm-3">
      ...
      <p>
        AgensGraph에 Northwind 데이터 적재가 완료되었다면 Cypher 쿼리로 데이터를 조회하여 데이터가 정상적으로
        적재되었는지 확인합니다. 이 튜토리얼에서는 이해를 돕기 위해 OpenBroswer에서 쿼리를 실행해 그래프를 시각화하여
        제공합니다.
      </p>
      <hr />
      <p>
...
LIMIT 100;`}
        />
        <aside className="warn">고객, 주문, 상품을 합쳐 최대 100개의 객체를 조회해보겠습니다.</aside>
      </figure>
